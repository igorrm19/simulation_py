...
def main():
    ...
    # --- Passo 1: Simulação Base (Antes da Otimização) ---
    ...
    # Desempenho em um SNR específico (ex: 15dB) antes da otimização
    snr_opt = 15
    ...
    # --- Passo 2: Otimização com Algoritmo Genético ---
    ...
    # Decodificando melhor indivíduo
    M_options = [4, 8, 16]
    mod_options = [2, 4, 16, 64]
    
    tx_power_opt = best_ind[0]
    relay_gain_opt = abs(best_ind[1]) # Garantir positivo por conta de mutação Gaussiana
    ...
    # --- Passo 3: Avaliação Final (Depois da Otimização) ---
    print("\n--- 3. Avaliação Final ---")
